...
if not os.path.isfile(ckpt_gan_path):
    ...
    checkpoint = torch.load(ckpt_gan_path, weights_only=True)
    if args.GAN_arch == "SAGAN":
        netG = sagan_generator(nz=args.dim_gan, dim_embed=args.dim_embed).cuda()
    else:
        netG = sngan_generator(nz=args.dim_gan, dim_embed=args.dim_embed).cuda()
...
if args.comp_FID:
    ...
    if args.comp_IS_and_FID_only:
        ...
        for i in range(len(centers_loc)):
            ...
            print("\r Center:{}; Real:{}; Fake:{}; FID:{:.3f}; LS:{:.3f}; ET:{:.3f}.".format(center, len(real_images_curr), len(fake_images_curr), FID_over_centers[i], labelscores_over_centers[i], entropies_over_centers[i]))

        # average over all centers
        print("\n {} SFID: {:.3f}({:.3f}); min/max: {:.3f}/{:.3f}.".format(args.GAN_arch, np.mean(FID_over_centers), np.std(FID_over_centers), np.min(FID_over_centers), np.max(FID_over_centers)))
        print("\n {} LS over centers: {:.3f}({:.3f}); min/max: {:.3f}/{:.3f}.".format(args.GAN_arch, np.mean(labelscores_over_centers), np.std(labelscores_over_centers), np.min(labelscores_over_centers), np.max(labelscores_over_centers)))
        print("\n {} entropy over centers: {:.3f}({:.3f}); min/max: {:.3f}/{:.3f}.".format(args.GAN_arch, np.mean(entropies_over_centers), np.std(entropies_over_centers), np.min(entropies_over_centers), np.max(entropies_over_centers)))
