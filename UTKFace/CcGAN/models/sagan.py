...
class sagan_generator(nn.Module):
    ...
    def __init__(self, nz=256, dim_embed=128, gen_ch=64):
        ...
        self.snlinear0 = snlinear(in_features=self.dim_z, out_features=gen_ch*16*4*4)
        self.block1 = GenBlock(gen_ch*16, gen_ch*8, dim_embed)
        self.block2 = GenBlock(gen_ch*8, gen_ch*4, dim_embed)
        self.block3 = GenBlock(gen_ch*4, gen_ch*2, dim_embed)
        self.self_attn = Self_Attn(gen_ch*2)
        self.block4 = GenBlock(gen_ch*2, gen_ch, dim_embed)
        self.bn = nn.BatchNorm2d(gen_ch)
        self.relu = nn.ReLU(inplace=True)
        self.snconv2d1 = snconv2d(in_channels=gen_ch, out_channels=3, kernel_size=3, stride=1, padding=1)
        self.tanh = nn.Tanh()
    ...
    def forward(self, z, labels):
        ...
        out = out.view(-1, self.gen_ch*16, 4, 4) # 4 x 4
...
if __name__ == "__main__":
    netG = sagan_generator(nz=256, dim_embed=128, gen_ch=64).cuda()
    netD = sagan_discriminator(dim_embed=128, disc_ch=64).cuda()
