...
if not os.path.isfile(Filename_GAN):
    ...
    checkpoint = torch.load(Filename_GAN, weights_only=True)
    if args.GAN_arch=="SNGAN":
        netG = CcGAN_SNGAN_Generator(dim_z=args.dim_gan, dim_embed=args.dim_embed)
    elif args.GAN_arch=="SAGAN":
        netG = CcGAN_SAGAN_Generator(dim_z=args.dim_gan, dim_embed=args.dim_embed)
...
if args.comp_FID:
    ...
    print("\r End sampling! We got {} fake images, which takes {:.3f} sec.".format(len(fake_images), timeit.default_timer()-start_time))
    ...
    if args.comp_IS_and_FID_only:
        ...
        for i in range(len(centers_loc)):
            ...
            ## Label score
            labelscores_over_centers[i], _ = cal_labelscore(PreNetLS, fake_images_curr, fn_norm_labels(fake_labels_assigned_curr), min_label_before_shift=0, max_label_after_shift=args.max_label, batch_size=200, resize = None, num_workers=args.num_workers)
            ## print
        ...
        print("\n SFID: {:.3f}({:.3f}); min/max: {:.3f}/{:.3f}.".format(np.mean(FID_over_centers), np.std(FID_over_centers), np.min(FID_over_centers), np.max(FID_over_centers)))
        print("\r LS over centers: {:.3f}({:.3f}); min/max: {:.3f}/{:.3f}.".format(np.mean(labelscores_over_centers), np.std(labelscores_over_centers), np.min(labelscores_over_centers), np.max(labelscores_over_centers)))
        print("\r Entropy over centers: {:.3f}({:.3f}); min/max: {:.3f}/{:.3f}.".format(np.mean(entropies_over_centers), np.std(entropies_over_centers), np.min(entropies_over_centers), np.max(entropies_over_centers)))
